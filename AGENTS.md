...
- PyKotor depends on the workspace package `bioware-kaitai-formats` (import name `bioware_kaitai_formats`). `pykotor.kaitai_generated` remains as thin shims for backward compatibility. Canonical `.ksy` sources still live in the separate `OpenKotOR/bioware-kaitai-formats` repo (clone into e.g. `vendor/bioware-kaitai-formats` for regeneration).
