...
All of the following are read with `CResGFF::ReadFieldSHORT(param_2, "FieldName", …)` in `ReadStatsFromGff` (/K1/K1_win_gog_swkotor.exe: @ 0x005afce0). Signed 16-bit range is -32768–32767; the engine uses non-negative values only, so UI min/max 0–32767.
...
|---------------|------------|---------------|------|-----|-----|-------|
| `FeatList` | `featList` | `Feat` | BYTE | 0 | 255 | Index into `feats.2da`. Each list element stores one feat ID. (/K1/k1_win_gog_swkotor.exe: ReadStatsFromGff @ 0x005afce0; SaveClassInfo writes. |
...
- [GFF-UTC](GFF-Creature-and-Dialogue#utc) -- Creature GFF structure
