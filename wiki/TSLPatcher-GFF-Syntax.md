...
- [GFFList Syntax — GFF File Modifications](#gfflist-syntax)
- [SSFList Syntax — Sound Set Modifications](#ssflist-syntax)
...
This guide explains how to modify [GFF files](GFF-File-Format) using TSLPatcher syntax. For the complete [GFF file](GFF-File-Format) format specification, see [GFF File Format](GFF-File-Format). For general TSLPatcher information, see [TSLPatcher's Official Readme](TSLPatcher's-Official-Readme). For HoloPatcher-specific information, see [HoloPatcher README for Mod Developers](HoloPatcher#mod-developers).
...
Some [GFF](GFF-File-Format) structures require the type ID to match the list index. Use `TypeId=ListIndex`:
...
Some [GFF](GFF-File-Format) structures require the STRUCT's type ID to match its position (index) in the list. This is common in:
...
### Example 3: [creature template](GFF-File-Format#utc-creature) with Dynamic data
...
- [HoloPatcher README for Mod Developers](HoloPatcher#mod-developers) - HoloPatcher-specific features and improvements
...
`2DAMEMORY#` tokens reference [2DA](2DA-File-Format) memory tokens that have been set elsewhere in changes.ini (typically from 2DAList entries). The stored value is converted to an integer stringref.
