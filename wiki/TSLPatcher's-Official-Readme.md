...
- It allows you to change values in existing cells in a [2DA file](2DA-File-Format) located in the override folder,
again ensuring compatibility with other mods that modify the same [2DA file](2DA-File-Format). (As long as
that mod does not modify that same cell, of course...)

- It allows you to add new columns to existing [2DA files](2DA-File-Format) in override, again helping to ensure
compatibility with other mods using the same files.

- It allows you to modify the values of existing fields within [GFF files](GFF-File-Format) located in the override
folder. Aside from allowing compatibility with other Mods using the same file, it also
allows you to update fields that refer to lines in a [2DA file](2DA-File-Format) to point to new lines the
...
- It is not able to make two Mods that make changes to the same standard game script ([NCS](NCS-File-Format))
...
2.5.Extract UNALTERED copies of any [2DA](2DA-File-Format) and standard game [GFF](GFF-File-Format) format files you want the
...
nwscript.nss into the tslpatchdata folder. Also copy the [NSS](NSS-File-Format) script files you intend to use
...
This section is where you list all the [2DA files](2DA-File-Format) the TSLPatcher should modify, and what
changes it should do to them. To add a file to modify, Select the [2DA](2DA-File-Format) files section in the tree
view and either choose Add [2DA](2DA-File-Format) file from the Modifiers menu, or right click on [2DA](2DA-File-Format) files in the
tree view and select Add [2DA](2DA-File-Format) file in the context menu.
...
list. You will now be prompted to first select an unaltered copy of the [2DA file](2DA-File-Format), suitably the one
found in your tslpatchdata folder.  Click Open and you will be prompted to select your
modified version of that same [2DA file](2DA-File-Format).
...
**Add line to [2DA](2DA-File-Format)**
What it sounds like. A new line will be added from scratch at the bottom of the [2DA file](2DA-File-Format).

**Modify line in [2DA](2DA-File-Format)**
Change the value in one or more columns for a line that already exists in the [2DA file](2DA-File-Format).

**Copy line in [2DA](2DA-File-Format)**
This creates a new line at the bottom of the [2DA file](2DA-File-Format) by making a copy of an already existing
row, letting you change the values in the relevant columns while keeping the other column
values from the original row.

**Add column to [2DA](2DA-File-Format)**
...
##### 3.3.2.1. Add [2DA](2DA-File-Format) line
...
When you have specified a Modifier, the Add [2DA](2DA-File-Format) line window will open. Here you can enter the
value to be set in each column for the new row.

The optional Exclusive Column box can be set to the name of a column in the [2DA](2DA-File-Format) file. This will
...
If you have a copy of the [2DA](2DA-File-Format) file in question in the same folder as your changes.ini file
ChangeEdit will load the column labels automatically for you into the Column dropdown list. If
not, press the Load... button to the right of the Column box and select a [2DA](2DA-File-Format) file in the open dialog
...
2DAMEMORY token, which can then be assigned to cells in other [2DA files](2DA-File-Format), or fields in [GFF](GFF-File-Format) files.
...
##### 3.3.2.2. Modify [2DA](2DA-File-Format) line
...
When you have specified a Modifier, the Modify [2DA](2DA-File-Format) line window will open. First set which
...
An extra, special case way of specifying the row to modify that only works for some [2DA files](2DA-File-Format)
is LabelIndex. This will look in the label column for a value. This obviously only works in
[2DA files](2DA-File-Format) which have a label column, and only works reliably in files where the value in the
...
Press the Load... button to the right of the Column box to load all the column labels from a [2DA](2DA-File-Format)
...
which can then be assigned to cells in other [2DA files](2DA-File-Format), or fields in [GFF](GFF-File-Format) files.

You can also assign the RowLabel or the name of any column to a 2DAMEMORY token. The
value in the [2DA file](2DA-File-Format) in that column is then saved in the 2DAMEMORY token for later use.

##### 3.3.2.3. Copy [2DA](2DA-File-Format) line
...
When you have specified a Modifier, the Copy [2DA](2DA-File-Format) line window will open. First select which
row you wish to make a copy of for your new line. You can specify this blueprint row either by
RowIndex (i.e. line number) of its RowLabel value (far-left column in KotORTool's [2DA](2DA-File-Format)
editor).
...
If you have a copy of the [2DA](2DA-File-Format) file in question in the same folder as your changes.ini file
ChangeEdit will load the column labels automatically for you into the Column dropdown list. If
not, press the Load... button to the right of the Column box and select a [2DA file](2DA-File-Format) in the open
...
KotORTool's [2DA](2DA-File-Format) editor) for your new row.
...
You can also assign the RowLabel or the name of any column to a 2DAMEMORY token. The
value for the new row in the [2DA file](2DA-File-Format) in that column is then saved in the 2DAMEMORY token for
later use.

##### 3.3.2.4. Add [2DA](2DA-File-Format) column
...
token, which can then be assigned to cells in other [2DA files](2DA-File-Format), or fields in [GFF](GFF-File-Format) files.

### 3.4. GFF files

[GFF](GFF-File-Format) is a file format used for a variety of different file types in the game, such at UT* templates,
[DLG](GFF-File-Format#dlg-dialogue) dialog files, the global.jrl [journal entry](GFF-File-Format#jrl-journal) file, just to mention a few.

TSLPatcher can modify the value of the fields in [GFF files](GFF-File-Format) found in the user's override folder (or
place a copy of the file there if it doesn't already exist before modifying it).

To do this, select the [GFF](GFF-File-Format) files section in the tree view and either choose Add [GFF](GFF-File-Format) file... from
the Modifiers menu, or right-click the section and choose Add [GFF](GFF-File-Format) file... from the context menu.
type in the name of the file to modify, be sure to include the file extension (i.e. .dlg, .uti
etc...). Then select the newly added [GFF file](GFF-File-Format) in the tree view to view its Modifier list.
...
To change the value of a [GFF](GFF-File-Format) field, type in the label of the field in the [GFF](GFF-File-Format) field box. If the field
...
from an existing [GFF file](GFF-File-Format). This may save you some typing and may help preventing typos in the
...
#### 3.4.2. Adding new [GFF](GFF-File-Format) fields

To Insert new fields into a [GFF file](GFF-File-Format), press the Manage new fields... button (icon with blue arrow
with a plus beneath) next to the red arrow icons in the [GFF](GFF-File-Format) Modifications panel.
...
about the supported [GFF](GFF-File-Format) field data types. Depending on what you select here, the remaining
...
This specifies where in the [GFF](GFF-File-Format) field tree you wish to add the new field. Leave this blank to add
...
LIST/STRUCT. This new window will work just like the main Editor window for the [GFF file](GFF-File-Format),
...
been modified. It will then compare the two and manufacture [GFF](GFF-File-Format) Modifiers from the things it
...
entry, and is not meant as a one-click solution for all manner of [GFF](GFF-File-Format) editing needs. Depending
...
box. Check the Replace existing box if you want the TSLPatcher to replace any [NCS files](NCS-File-Format) with the
same name already existing in the at the destination when installing. If this is unchecked and
the [NCS](NCS-File-Format) already exists in at the destination a warning will be posted in the TSLPatcher
...
In the [NSS](NSS-File-Format) scripts themselves, wherever you want a value to be inserted, type in the name of
...
The nwscript.[NSS files](NSS-File-Format) can be found within the scripts.bif file in the game data and
...
or carriage return characters when patching [GFF](GFF-File-Format) format files. Earlier the INI
...
label and data type already exist at that location in the [GFF file](GFF-File-Format). TSLPatcher will
...
Updated ChangeEdit's "New [GFF](GFF-File-Format) field" editor with a FieldPath token input
...
2006-03-25
Updated ChangeEdit with a [GFF](GFF-File-Format) Compare function and a button to copy
...
2006-01-09
 ...
 2006-01-10
Added functionality for the Patcher to add new fields to [GFF files](GFF-File-Format).

 2006-01-12
Added functionality for the Patcher process [NSS files](NSS-File-Format) for tokens and
...
value in that specified column already exist in the [2DA file](2DA-File-Format).
