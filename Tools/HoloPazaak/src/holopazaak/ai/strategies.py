...
class NormalAI(AIStrategy):
    ...
    def _should_stay(self, player: Player, opponent: Player, score: int, opp_score: int) -> bool:
...
class HardAI(AIStrategy):
    ...
    def decide(self, game: PazaakGame, player: Player) -> AIDecision:
        ...
        if opponent.is_standing:
            return self._handle_opponent_standing(player, score, opp_score, safe_to_draw)
        else:
            return self._handle_opponent_playing(player, score, opp_score, safe_to_draw)
    ...
    def _handle_bust(self, player: Player, score: int, opp_score: int) -> AIDecision:
    ...
    def _find_stand_card(self, player: Player, score: int) -> AIDecision | None:
...
class ExpertAI(AIStrategy):
    ...
    def _handle_opponent_standing(self, player: Player, score: int, opp_score: int) -> AIDecision:
        """Handle when opponent is standing."""
        # If winning, stand
        if score > opp_score and score <= 20:
            return AIDecision(AIAction.STAND)
    ...
    def _handle_opponent_playing(self, player: Player, score: int, opp_score: int) -> AIDecision:
        """Handle when opponent is still playing."""
