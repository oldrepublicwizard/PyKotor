...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.plotCheckbox.setToolTip(_translate("MainWindow", "Plot (GFF: Plot). BYTE 0/1. Engine reads via ReadFieldBYTE in LoadDataFromGff (K1 @ 0x0055fcd0). When set, the item cannot be dropped or sold; used for quest and story-critical items. Modders: enable for items that must stay in inventory; NPCs with Plot items will not drop them on death."))
        self.label_43.setText(_translate("MainWindow", "Charges:"))
        self.chargesSpin.setToolTip(_translate("MainWindow", "Charges (GFF: Charges). BYTE 0–255. Engine reads via ReadFieldBYTE in LoadDataFromGff (K1 @ 0x0055fcd0); default 50 (0x32) when missing. Current uses remaining for usable items (medpacs, grenades, etc.). Modders: 0 = empty/unusable; max 255. When charges reach 0, the item is consumed or unusable until recharged."))
        self.label_44.setText(_translate("MainWindow", "Stack Size:"))
        self.stackSpin.setToolTip(_translate("MainWindow", "Stack Size (GFF: StackSize). WORD 0–65535. Engine reads via ReadFieldWORD in LoadDataFromGff (K1 @ 0x0055fcd0). Max number of this item that can stack in one inventory slot. Modders: 1 = non-stackable (weapons, armor); higher for consumables/ammo. Very high values work but may overflow in some UIs."))
        self.label_45.setText(_translate("MainWindow", "Model Variation:"))
        self.modelVarSpin.setToolTip(_translate("MainWindow", "Model Variation (GFF: ModelVariation). BYTE 0–255. Engine reads via ReadFieldBYTE in LoadDataFromGff (K1 @ 0x0055fcd0); if 0, ModelPart1 is read as fallback (and clamped to 1 if still 0). Selects alternate meshes from the base item. Modders: check baseitems.2da and model files; invalid indices may show default or wrong visuals."))
        self.label_46.setText(_translate("MainWindow", "Body Variation:"))
        self.bodyVarSpin.setToolTip(_translate("MainWindow", "Body Variation (GFF: BodyVariation). Used for armor (model_type 1) only. In LoadDataFromGff (K1 @ 0x0055fcd0), when base item is armor the engine sets body_variation from the base item\'s body_var and then reads TextureVar from GFF; BodyVariation may be stored for toolset/override. Modders: only applies to armor; check appearance.2da and armor models. Valid 0–255."))
        self.label_47.setText(_translate("MainWindow", "Texture Variation:"))
        self.textureVarSpin.setToolTip(_translate("MainWindow", "Texture Variation (GFF: TextureVar). BYTE 0–255. Engine reads via ReadFieldBYTE in LoadDataFromGff (K1 @ 0x0055fcd0) only when base item model_type is 1 (armor); default 1 when missing. Selects texture variant for armor. Modders: only applies to armor; check armor textures for valid indices."))
        self.groupBox.setTitle(_translate("MainWindow", "Icon"))
        self.iconLabel.setToolTip(_translate("MainWindow", "Item icon preview. The game derives the icon from the base item (baseitems.2da) and sometimes model; this is display-only. Change Base Item or model variation to change the icon. Not stored as a separate GFF field."))
