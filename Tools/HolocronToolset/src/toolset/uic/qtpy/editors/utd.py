...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.previewRenderer.setToolTip(_translate("MainWindow", "Door model preview. Displays the 3D model for the selected Appearance (Generic Doors 2DA row). The game uses the same appearance index when loading the door in-game. Drag to rotate; use the Appearance dropdown to change which door model is shown. Not a GFF field—purely for visual feedback in the editor."))
        self.modelInfoGroupBox.setToolTip(_translate("MainWindow", "Model information panel. Expand to see details about the door model (mesh count, animations, etc.) derived from the Appearance and game resources. Read-only; does not change the UTD file. Useful to verify the correct model loads for the chosen Appearance."))
        self.modelInfoGroupBox.setTitle(_translate("MainWindow", "Model Info"))
        self.modelInfoSummaryLabel.setToolTip(_translate("MainWindow", "Short summary of the door model (e.g. mesh and animation count). Click the Model Info group to expand and see full details. Derived from the current Appearance; not a GFF field."))
        self.modelInfoSummaryLabel.setText(_translate("MainWindow", "Click to expand model information..."))
        self.modelInfoLabel.setToolTip(_translate("MainWindow", "Detailed model information (meshes, animations, bounds) for the door. Shown when Model Info is expanded. Helps modders confirm the door appearance matches expectations. Not stored in the UTD GFF."))
        ...
        self.nameEdit.setToolTip(_translate("MainWindow", "Name (GFF: LocName). Localized name shown when the player examines the door or in dialogue. The game reads this via ReadFieldCExoLocString in LoadDoor. Modders: set a clear name for your door; use the language dropdown to add translations."))
        self.label_14.setText(_translate("MainWindow", "Tag:"))
        self.tagEdit.setToolTip(_translate("MainWindow", "Tag (GFF: Tag). CExoString. Unique identifier for this door; scripts and dialogues use GetObjectByTag to find it. Engine reads via ReadFieldCExoString in LoadDoor. Modders: keep unique per module; use a descriptive tag (e.g. \"tar_maze_door\")."))
        self.tagGenerateButton.setToolTip(_translate("MainWindow", "Generate tag from ResRef. Copies the Template ResRef into the Tag field so they stay in sync. Scripts reference doors by Tag; keep Tag unique per module."))
        self.tagGenerateButton.setText(_translate("MainWindow", "-"))
        self.label_38.setText(_translate("MainWindow", "ResRef:"))
        self.resrefEdit.setToolTip(_translate("MainWindow", "Template ResRef (GFF: TemplateResRef). ResRef (max 16 characters) identifying this door template. The engine stores ResRefs in a 16-byte structure. When the game loads a door from a GIT file it uses this to find the UTD template. Modders: set to match the UTD filename (without .utd); keep under 16 characters."))
        self.resrefGenerateButton.setToolTip(_translate("MainWindow", "Generate ResRef from Tag. Copies the Tag into the Template ResRef field (max 16 characters). ResRef should match the UTD filename."))
        self.resrefGenerateButton.setText(_translate("MainWindow", "-"))
        self.label_15.setText(_translate("MainWindow", "Appearance:"))
        self.appearanceSelect.setToolTip(_translate("MainWindow", "Appearance (GFF: Appearance). DWORD read as byte in LoadDoor—index into doortypes.2da or genericdoors.2da. Picks the 3D model and behavior of the door. Modders: choose a row that exists in the door 2DA for your game; invalid indices may show a wrong or missing model."))
        self.label.setText(_translate("MainWindow", "Conversation:"))
        self.conversationEdit.setToolTip(_translate("MainWindow", "Conversation (GFF: Conversation). ResRef of the dialogue (DLG) file; max 16 characters. When the player uses this door the game can run this dialogue. Blank means no dialogue. Engine reads via ReadFieldCResRef in LoadDoor. Modders: set to a DLG resref; use Edit to open the dialogue editor."))
        ...
        self.min1HpCheckbox.setToolTip(_translate("MainWindow", "Min1HP (GFF: Min1HP). BYTE 0/1. When set, the door cannot be reduced below 1 HP by damage—it stays at 1 until destroyed by script or other means. Engine reads via ReadFieldBYTE in LoadDoor. KotOR 2 only in some builds. Modders: enable for doors that should be nearly unbreakable."))
        self.min1HpCheckbox.setText(_translate("MainWindow", "Min1HP"))
        self.plotCheckbox.setToolTip(_translate("MainWindow", "Plot (GFF: Plot). BYTE 0/1. When set, the door is plot-critical: it cannot be destroyed (treated as invulnerable if Static is not set). Engine reads Plot/Invulnerable/Static in LoadDoor and sets object.plot. Modders: enable for story doors that must never be broken."))
        self.plotCheckbox.setText(_translate("MainWindow", "Plot"))
        self.partyInteractCheckbox.setToolTip(_translate("MainWindow", "Party Interact. When set, the party can interact with this door (e.g. open, use dialogue). Used by the toolset/scripts; behavior may vary by game. Modders: enable for doors the player should be able to use."))
        self.partyInteractCheckbox.setText(_translate("MainWindow", "Party Interact"))
        self.staticCheckbox.setToolTip(_translate("MainWindow", "Static (GFF: Static). BYTE 0/1. When set, the door is non-interactive (no open/close, no dialogue). Engine reads via ReadFieldBYTE in LoadDoor; if Static is 0, Plot flag still makes it invulnerable. Modders: use for decorative or script-only doors."))
        self.staticCheckbox.setText(_translate("MainWindow", "Static"))
        self.useableCheckbox.setToolTip(_translate("MainWindow", "Useable. When set, the door can be used (opened, conversation). Modders: enable for normal doors the player can interact with; disable for decoration or doors opened only by script."))
        self.useableCheckbox.setText(_translate("MainWindow", "Useable"))
        self.notBlastableCheckbox.setToolTip(_translate("MainWindow", "Not Blastable (GFF: NotBlastable). KotOR 2 only. When set, the door cannot be blasted open (e.g. with explosives). Modders: enable for doors that must be unlocked or opened by key/script only."))
        ...
        self.factionSelect.setToolTip(_translate("MainWindow", "Faction (GFF: Faction). DWORD. Faction identifier; determines hostility and who can use the door. Engine reads via ReadFieldDWORD in LoadDoor. Modders: set to match your module\'s factions (repute.2da); wrong values can make the door hostile or unusable by the party."))
        self.label_42.setText(_translate("MainWindow", "Animation State:"))
        self.animationState.setToolTip(_translate("MainWindow", "Animation State (GFF: AnimationState). BYTE 0–255. Current door animation state (e.g. open/closed). K1 CSWSDoor::LoadDoor @ 0x0058a1f0 does not read this field in the template loader; it may be used when loading from save or for instance state in the area. In template UTDs leave at 0; runtime state is stored in the GIT for placed instances. Modders: set 0 for new doors."))
        self.label_3.setText(_translate("MainWindow", "Current HP:"))
        self.currentHpSpin.setToolTip(_translate("MainWindow", "Current HP (GFF: CurrentHP). SHORT. Door\'s current hit points when loaded; when it reaches 0 the door is destroyed and OnDeath runs. Engine reads via ReadFieldSHORT in LoadDoor. Modders: valid range 0–32767; usually set equal to Max HP for a full-strength door. Clamped to Max HP when loading from GIT if a flag is set."))
        self.label_26.setText(_translate("MainWindow", "Max HP:"))
        self.maxHpSpin.setToolTip(_translate("MainWindow", "Max HP (GFF: HP). SHORT. Maximum hit points; door is destroyed when Current HP reaches 0. Engine reads via ReadFieldSHORT in LoadDoor. Modders: valid range 0–32767; typical values 10–50 for breakable doors. Plot or Static doors may ignore damage."))
        self.label_20.setText(_translate("MainWindow", "Hardness:"))
        self.hardnessSpin.setToolTip(_translate("MainWindow", "Hardness (GFF: Hardness). BYTE. Damage reduction applied when the door is hit; reduces each attack\'s damage. Engine reads via ReadFieldBYTE in LoadDoor (default 0). Modders: valid range 0–255; typical 0–20. Does not prevent destruction when HP reaches 0."))
        self.label_21.setText(_translate("MainWindow", "Fortitude:"))
        self.fortitudeSpin.setToolTip(_translate("MainWindow", "Fortitude (GFF: Fort). BYTE. Fortitude save value for the door; used in some save checks (e.g. against spells). Engine reads via ReadFieldBYTE in LoadDoor. Modders: valid range 0–255; often 0 in vanilla files."))
        self.label_24.setText(_translate("MainWindow", "Reflex:"))
        self.reflexSpin.setToolTip(_translate("MainWindow", "Reflex (GFF: Ref). BYTE. Reflex save value for the door; used in some save checks. Engine reads via ReadFieldBYTE in LoadDoor. Modders: valid range 0–255; often 0 in vanilla files."))
        self.label_25.setText(_translate("MainWindow", "Will:"))
        self.willSpin.setToolTip(_translate("MainWindow", "Will (GFF: Will). BYTE. Will save value for the door; used in some save checks (e.g. against mind-affecting effects). Engine reads via ReadFieldBYTE in LoadDoor. Modders: valid range 0–255; often 0 in vanilla files."))
        ...
        self.needKeyCheckbox.setToolTip(_translate("MainWindow", "Key Required (GFF: KeyRequired). BYTE 0/1. When set, the door is locked until the player uses an item whose tag matches Key Tag. Engine reads via ReadFieldBYTE in LoadDoor. Modders: enable and set Key Tag to the tag of the key item (e.g. a UTI in the module)."))
        self.needKeyCheckbox.setText(_translate("MainWindow", "Key required to unlock"))
        self.removeKeyCheckbox.setToolTip(_translate("MainWindow", "Remove Key on Unlock (GFF: AutoRemoveKey). BYTE 0/1. When set, the key item is consumed (removed from inventory) when used to unlock the door. Engine reads via ReadFieldBYTE in LoadDoor. Modders: enable for one-use keys; disable if the key should remain in inventory."))
        ...
        self.keyEdit.setToolTip(_translate("MainWindow", "Key Tag (GFF: KeyName). CExoString. Tag of the key item required to unlock the door when Key Required is set. The player must have an item in inventory whose tag matches this string. Engine reads via ReadFieldCExoString in LoadDoor. Modders: use the same tag as the key UTI\'s Tag field."))
        self.groupBox_2.setTitle(_translate("MainWindow", "Lock"))
        self.lockedCheckbox.setToolTip(_translate("MainWindow", "Locked (GFF: Locked). BYTE 0/1. When set, the door starts locked; the player must use a matching key or pick the lock (Open Lock DC) to open it. Modders: enable for doors that should start locked; combine with Key Required and Key Tag for key-only doors."))
        ...
        self.openLockSpin.setToolTip(_translate("MainWindow", "Open Lock DC (GFF: OpenLockDC). BYTE. Difficulty class to pick the lock (Security check). Engine reads via ReadFieldBYTE in LoadDoor. Modders: valid range 0–255; typical 5–25. Use 0 for unlocked or non-lockable doors. Key Required overrides this when the key is used."))
        self.difficultySpin.setToolTip(_translate("MainWindow", "Difficulty (GFF: OpenLockDiff). KotOR 2 only. Additional difficulty value for lock picking. Modders: valid range 0–255; only used in TSL. Leave 0 if targeting K1."))
        self.difficultyModSpin.setToolTip(_translate("MainWindow", "Difficulty Mod (GFF: OpenLockDiffMod). KotOR 2 only. INT8 (-128 to 127). Modifier added to Open Lock DC when calculating lock pick difficulty; positive values make the lock harder, negative values make it easier. Engine reads via ReadField in TSL LoadDoor. Modders: use 0 for default; positive for tougher locks, negative to soften difficulty. Leave 0 if targeting K1."))
        ...
        self.onClickEdit.setToolTip(_translate("MainWindow", "OnClicked (GFF: OnClick). ResRef of NCS script (max 16 chars). Fires when the player clicks the door. Leave blank for no script. Modders: use for custom door interaction or dialogue triggers."))
        self.label_5.setText(_translate("MainWindow", "OnClosed:"))
        self.onClosedEdit.setToolTip(_translate("MainWindow", "OnClosed (GFF: OnClosed). ResRef of NCS script. Fires when the door closes. Engine reads via ReadFieldCResRef in LoadDoor. Leave blank for no script. Max 16 characters."))
        self.label_7.setText(_translate("MainWindow", "OnDamaged:"))
        self.onDamagedEdit.setToolTip(_translate("MainWindow", "OnDamaged (GFF: OnDamaged). ResRef of NCS script. Fires when the door takes damage. Engine reads via ReadFieldCResRef in LoadDoor. Leave blank for no script. Max 16 characters."))
        self.label_8.setText(_translate("MainWindow", "OnDeath:"))
        self.onDeathEdit.setToolTip(_translate("MainWindow", "OnDeath (GFF: OnDeath). ResRef of NCS script. Fires when the door is destroyed (HP reaches 0). Engine reads via ReadFieldCResRef in LoadDoor. Leave blank for no script. Max 16 characters."))
        self.label_10.setText(_translate("MainWindow", "OnOpenFailed:"))
        self.onOpenFailedEdit.setToolTip(_translate("MainWindow", "OnOpenFailed. ResRef of NCS script. Fires when the player fails to open the door (e.g. failed lock pick). Leave blank for no script. Max 16 characters."))
        self.label_11.setText(_translate("MainWindow", "OnHeartbeat:"))
        self.onHeartbeatSelect.setToolTip(_translate("MainWindow", "OnHeartbeat (GFF: OnHeartbeat). ResRef of NCS script. Fires periodically while the door is in the area. Engine reads via ReadFieldCResRef in LoadDoor. Leave blank for no script. Max 16 characters."))
        self.label_13.setText(_translate("MainWindow", "OnMeleeAttack:"))
        self.onMeleeAttackEdit.setToolTip(_translate("MainWindow", "OnMeleeAttack. ResRef of NCS script. Fires when the door is melee-attacked. Leave blank for no script. Max 16 characters."))
        self.label_16.setText(_translate("MainWindow", "OnSpellCastAt:"))
        self.onSpellEdit.setToolTip(_translate("MainWindow", "OnSpellCastAt (GFF: OnSpellCastAt). ResRef of NCS script. Fires when a spell is cast at the door. Leave blank for no script. Max 16 characters."))
        self.label_17.setText(_translate("MainWindow", "OnOpen:"))
        self.onOpenEdit.setToolTip(_translate("MainWindow", "OnOpen. ResRef of NCS script. Fires when the door is opened. Leave blank for no script. Max 16 characters."))
        self.label_23.setText(_translate("MainWindow", "OnUnlock:"))
        self.onUnlockEdit.setToolTip(_translate("MainWindow", "OnUnlock. ResRef of NCS script. Fires when the door is unlocked (key or lock pick). Leave blank for no script. Max 16 characters."))
        self.label_19.setText(_translate("MainWindow", "OnUserDefined:"))
        self.onUserDefinedSelect.setToolTip(_translate("MainWindow", "OnUserDefined (GFF: ScriptUserDefine). ResRef of NCS script. Fires when another script calls ExecuteScript with this door and the user-defined event. Leave blank for no script. Max 16 characters."))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab_2), _translate("MainWindow", "Scripts"))
        self.commentsEdit.setToolTip(_translate("MainWindow", "Comments (GFF: Comment). CExoString. Developer notes only—the game never reads or displays this. Use it to document the door (e.g. purpose, author, change log). Modders: add notes for yourself or your team."))
