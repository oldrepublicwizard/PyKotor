...
NCS files contain compiled NWScript bytecode used in **KotOR and TSL**. The source of these scripts are stored in the `.nss` extension. While your game installation may have a bunch of `.nss` files in various `.bif` files, none of them are actually used by the game at runtime only the `.ncs` are. Scripts run inside a stack-based virtual machine **shared across Aurora engine games** (*KotOR*, *Neverwinter Nights*, etc.). *KotOR* inherits the same format with minor opcode additions for game-specific systems. **This documentation focuses on *KotOR*-specific behavior**, though the core format is shared with *Neverwinter Nights*.
...
**Reference**: [`vendor/xoreos-docs/specs/torlack/ncs.html`](https://github.com/th3w1zard1/xoreos-docs/blob/master/specs/torlack/ncs.html) - Tim Smith (Torlack)'s comprehensive [NwScript](NSS-File-Format) bytecode documentation with detailed opcode tables, stack operation examples, and subroutine calling conventions

### Stack-Based Virtual Machine

[NwScript](NSS-File-Format) uses a stack-based *VM* where all operations work on a stack rather than CPU registers. Stack grows downward with negative offsets, 4-[byte](https://en.wikipedia.org/wiki/Byte) aligned elements.
...
1. file signature: `"NCS V1.0"` (bytes `[0x4E, 0x43, 0x53, 0x20, 0x56, 0x31, 0x2E, 0x30]` ASCII bytes)
...
PyKotor reuses the [NwScript](NSS-File-Format) opcode table; each opcode accepts a specific qualifier/argument layout described below.
...
- `CONSTS` (0x05): `[0x04][0x05][uint16 len][ASCII]` (2+N B), SP+4 (pointer only, content stored separately), no [null terminator](https://en.cppreference.com/w/c/string/byte) (ASCII encoding)
...
All jump instructions use **[relative offsets](https://en.wikipedia.org/wiki/Relative_address)** from the start of the jump instruction itself:
