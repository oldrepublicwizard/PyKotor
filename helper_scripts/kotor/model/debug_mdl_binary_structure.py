...
if tex_pos != -1:
    ...
    print(f"  mdx_data_bitmap: 0x{bitmap_orig:08X} (TEXTURE1: {bool(bitmap_orig & 0x2)})")
    ...
    if mdx_data_off_orig < len(orig_mdx_data):
        ...
        if mdx_vertex_off_orig < len(orig_mdx_data) - mdx_data_off_orig:
            ...
            if vpos_offset + 12 <= len(orig_mdx_data):
                x, y, z = struct.unpack("<fff", orig_mdx_data[vpos_offset : vpos_offset + 12])
                print(f"({x:.3f}, {y:.3f}, {z:.3f})")
        if tex1_off_orig != 0xFFFFFFFF and tex1_off_orig < len(orig_mdx_data) - mdx_data_off_orig:
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    if tex_pos_gen != -1:
        ...
        print(f"  mdx_data_bitmap: 0x{bitmap_gen:08X} (TEXTURE1: {bool(bitmap_gen & 0x2)})")
