...
REPLACEMENT_RULES.extend(
    [
        ...
        ReplacementRule(
            r"\bmesh\b(?!\s*\([^)]*\))", "[mesh](MDL-MDX-File-Format#trimesh-header)", LinkCategory.CROSS_REFERENCES, priority=5, context_check=should_skip_compound_term
        ),
        ReplacementRule(
            r"\bmeshes\b(?!\s*\([^)]*\))", "[meshes](MDL-MDX-File-Format#trimesh-header)", LinkCategory.CROSS_REFERENCES, priority=5, context_check=should_skip_compound_term
        ),
...
REPLACEMENT_RULES.extend(
    [
        ...
        ReplacementRule(
            r"\bnull-terminated string\b(?!\s*\([^)]*\))", "[null-terminated string](https://en.cppreference.com/w/c/string/byte)", LinkCategory.TECHNICAL_TERMS, priority=10
        ),
...
REPLACEMENT_RULES.extend(
    [
        ...
        ReplacementRule(
            r"\bgame instance template\b(?!\s*\([^)]*\))", "[game instance template](GFF-File-Format#git-game-instance-template)", LinkCategory.GAME_TERMS, priority=10
        ),
        ReplacementRule(
            r"\bgame instance templates\b(?!\s*\([^)]*\))", "[game instance templates](GFF-File-Format#git-game-instance-template)", LinkCategory.GAME_TERMS, priority=10
        ),
...
REPLACEMENT_RULES.extend(
    [
        ReplacementRule(
            r"\[null byte\]\(https://en\.wikipedia\.org/wiki/Null-terminated_string\)",
            "[null byte](https://en.cppreference.com/w/c/string/byte)",
            LinkCategory.WIKIPEDIA_REPLACEMENTS,
            priority=20,
        ),
        ReplacementRule(
            r"\[null terminator\]\(https://en\.wikipedia\.org/wiki/Null-terminated_string\)",
            "[null terminator](https://en.cppreference.com/w/c/string/byte)",
            LinkCategory.WIKIPEDIA_REPLACEMENTS,
            priority=20,
        ),
        ReplacementRule(
            r"\[null-terminated string\]\(https://en\.wikipedia\.org/wiki/Null-terminated_string\)",
            "[null-terminated string](https://en.cppreference.com/w/c/string/byte)",
            LinkCategory.WIKIPEDIA_REPLACEMENTS,
            priority=20,
        ),
        ReplacementRule(
            r"\[null-terminated\]\(https://en\.wikipedia\.org/wiki/Null-terminated_string\)",
            "[null-terminated](https://en.cppreference.com/w/c/string/byte)",
            LinkCategory.WIKIPEDIA_REPLACEMENTS,
            priority=20,
        ),
...
def main() -> int:
    ...
    # Filter rules by category
    include_categories = [LinkCategory(cat) for cat in args.include] if args.include else None
    exclude_categories = [LinkCategory(cat) for cat in args.exclude] if args.exclude else None
    rules = filter_rules_by_category(REPLACEMENT_RULES, include_categories, exclude_categories)
    ...
    files_to_process = get_files_to_process(wiki_dir, args.files, args.skip_patterns)
    ...
    for md_file in files_to_process:
        ...
        files_changed, replacements = process_file(md_file, rules, dry_run=args.dry_run, verbose=args.verbose)
