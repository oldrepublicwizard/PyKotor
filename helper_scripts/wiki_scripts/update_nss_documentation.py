...
def get_function_file_and_anchor(
    ...
    Args:
        ...
        category: Category name (e.g., "Actions", "Class System")
        section_type: "Shared Functions (K1 & TSL)", "K1-Only Functions", or "TSL-Only Functions"

    Returns:
        (filename_without_ext, anchor) or None if not found in extracted files
    """
    project_root = Path(__file__).parent.parent
    wiki_dir = project_root / "wiki"

    # Determine prefix based on section type
    if section_type == "Shared Functions (K1 & TSL)":
        prefix = "NSS-Shared-Functions"
    elif section_type == "K1-Only Functions":
        prefix = "NSS-K1-Only-Functions"
    elif section_type == "TSL-Only Functions":
        prefix = "NSS-TSL-Only-Functions"
    else:
        return None

    # Build expected filename
    category_clean = category.replace(" ", "-")
    expected_file = wiki_dir / f"{prefix}-{category_clean}.md"

    if expected_file.exists():
        # Read file to find anchor for this function
        content = expected_file.read_text(encoding="utf-8")
        # Look for anchor followed by function name
        anchor_pattern = rf'<a id="([^"]+)"></a>\s*\n\s*##\s+`{re.escape(func_name)}\('
        match = re.search(anchor_pattern, content)
        if match:
            anchor = match.group(1)
            return (expected_file.stem, anchor)
...
def generate_toc_entry(
    ...
    Args:
        level: Heading level (2, 3, 4, etc.)
        title: Display title (function name or category name)
        is_item: True if this is a function/constant item (level 4)
        func_name: Function/constant name (if is_item=True)
        category: Category name (if is_item=True)
        section_type: Section type (if is_item=True)
        routine_num: Routine number (if is_item=True and available)
    """
    if is_item and func_name and category and section_type:
        # Try to find the function in extracted files
        result = get_function_file_and_anchor(func_name, category, section_type)
        if result:
            file_name, anchor = result
            indent = "      "  # 6 spaces for items under subsections
            routine_str = f" - Routine {routine_num}" if routine_num else ""
            return f"{indent}- [`{title}`{routine_str}]({file_name}#{anchor})"

    # Fallback to anchor-based link (for non-extracted sections or when extraction not found)
    anchor = title_to_anchor(title)
    if is_item:
        indent = "      "  # 6 spaces for items under subsections
        routine_str = f" - Routine {routine_num}" if routine_num else ""
        return f"{indent}- [`{title}`{routine_str}](#{anchor})"
    else:
...
def build_section_content(
    ...
    toc_entries: list[tuple[str, bool, str | None, str | None, int | None]] = []
    ...
    for category in sorted(items_by_category.keys()):
        ...
        toc_entries.append((category, False, None, None, None))
        ...
        for item in items_sorted:
            ...
            if item_type == "function":
                ...
                # Store routine number for TOC generation
                toc_entries.append((item.name, True, category, section_type, routine_num))
            else:
                item_md = format_func(item, refs_md)
                toc_entries.append((item.name, True, category, section_type, None))
...
def main():
    ...
    k1_func_content, toc_entries_by_section["K1-Only Functions"] = build_section_content(
        k1_only_funcs, format_function_markdown, "function", vendor_to_repo, "K1-Only Functions"
    )

    print("\n[3/6] Processing TSL-Only Functions...")
    tsl_func_content, toc_entries_by_section["TSL-Only Functions"] = build_section_content(
        tsl_only_funcs, format_function_markdown, "function", vendor_to_repo, "TSL-Only Functions"
    )
    ...
    k1_const_content, toc_entries_by_section["K1-Only Constants"] = build_section_content(
        k1_only_consts, format_constant_markdown, "constant", vendor_to_repo, "K1-Only Constants"
    )

    print("\n[6/6] Processing TSL-Only Constants...")
    tsl_const_content, toc_entries_by_section["TSL-Only Constants"] = build_section_content(
        tsl_only_consts, format_constant_markdown, "constant", vendor_to_repo, "TSL-Only Constants"
    )
    ...
    for section_name in [
        ...
        if section_name in toc_entries_by_section:
            for entry in toc_entries_by_section[section_name]:
                if isinstance(entry, tuple) and len(entry) == 5:
                    name, is_item, category, entry_section_type, routine_num = entry
                elif len(entry) == 4:
                    name, is_item, category, entry_section_type = entry
                    routine_num = None
                else:
                    # Legacy format: (name, is_item)
                    name, is_item = entry
                    category = None
                    entry_section_type = None
                    routine_num = None
