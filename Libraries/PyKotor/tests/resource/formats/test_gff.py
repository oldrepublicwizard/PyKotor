...
class TestGFF(TestCase):
    ...
    def validate_io(self, gff: GFF):
        ...
        assert locstring.get(Language.ENGLISH, Gender.MALE) == "male_eng", (
            "Locstring get(Language.ENGLISH, Gender.MALE) {locstring.get(Language.ENGLISH, Gender.MALE)} is not 'male_eng'"
        )
        assert locstring.get(Language.GERMAN, Gender.FEMALE) == "fem_german", (
            "Locstring get(Language.GERMAN, Gender.FEMALE) {locstring.get(Language.GERMAN, Gender.FEMALE)} is not 'fem_german'"
        )
