...
class TestWAVData(unittest.TestCase):
    ...
    def test_wav_equality(self):
        ...
        wav1 = WAV(wav_type=WAVType.VO, channels=1, sample_rate=44100, data=b"test")
        wav2 = WAV(wav_type=WAVType.VO, channels=1, sample_rate=44100, data=b"test")
        wav3 = WAV(wav_type=WAVType.SFX, channels=1, sample_rate=44100, data=b"test")
...
class TestWAVIO(unittest.TestCase):
    ...
    def test_read_raises_missing_format_chunk(self):
        ...
        invalid_data = b"RIFF" + struct.pack("<I", 36) + b"WAVE" + b"XXXX" + b"\x00" * 28
        self.assertRaises(ValueError, read_wav, invalid_data)
...
class TestWAVEdgeCases(unittest.TestCase):
    ...
    def test_wav_with_large_data(self):
        ...
        wav = WAV(channels=1, sample_rate=44100, bits_per_sample=16, data=large_data)
    ...
    def test_wav_with_various_channels(self):
        ...
        for channels in [1, 2]:
            with self.subTest(channels=channels):
                ...
                wav = WAV(channels=channels, sample_rate=44100, bits_per_sample=16, data=audio_data)
    ...
    def test_wav_with_various_sample_rates(self):
        ...
        for rate in sample_rates:
            with self.subTest(sample_rate=rate):
                wav = WAV(sample_rate=rate, data=b"\x00\x01" * 1000)
