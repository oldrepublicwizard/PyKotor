...
if __name__ == "__main__":
    ...
    def test_io_performance(  # noqa: C901, PLR0912, PLR0915
        ...
        for i in range(NUM_INSTANTIATIONS):
            try:
                ...
                if stream_class is RawBinaryReader:
                    if mode == "file":
                        stream = RawBinaryReader.from_file(TEST_FILE)
                    elif mode == "bytes":
                        ...
                        instantiation_start_time = time.time()
                        stream = RawBinaryReader.from_stream(raw_stream)
                        ...
                        instantiation_start_time = time.time()
                        stream = RawBinaryReader.from_stream(stream)
    ...
    def main():
        ...
        stream_types: list[tuple[type, str]] = [
            ...
            (RawBinaryReader, "file"),
            (RawBinaryReader, "mmap"),
            (RawBinaryReader, "stream(io.BufferedReader)"),
            (RawBinaryReader, "stream(io.BufferedRandom)"),
            (RawBinaryReader, "stream(io.BytesIO)"),
            (RawBinaryReader, "stream(io.FileIO)"),
            (RawBinaryReader, "stream(raw)"),
        ]
